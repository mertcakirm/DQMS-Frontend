  ...
  const handlePrint = () => {
    ...
    const printableContent = `
        ...
        <head>
            ...
            <title>Tedarikçi Değerlendirme Formu</title>
            <style>
                body {
        ...
        <body>
            ...
            <table class="header-table">
                <tr>
                    ...
                    <td colspan="4" style="font-size: 1.5em; text-align: center;">Tedarikçi Değerlendirme Formu</td>
            ...
            <table>
               <thead>
                    <tr>
                        <th rowspan="2" style="width: 50px;">Tedarikçi Firma</th>
                        <th rowspan="2" style="width: 50px;">Hız</th>
                        <th rowspan="2" style="width: 50px;">Hizmet Kalitesi</th>
                        <th rowspan="2" style="width: 50px;">Güvenilirlik</th>
                        <th rowspan="2" style="width: 50px;">Termin Süresi</th>
                        <th rowspan="2" style="width: 50px;">(0-10)</th>
                        <th rowspan="2" style="width: 50px;">Toplam Puan</th>
                        <th rowspan="2" style="width: 50px;">Ortalama Puan</th>
                        <th rowspan="2" style="width: 50px;">%100 Hesaplama</th>
                        <th rowspan="2" style="width: 50px;">Değerlendirmeyi Yapan</th>
                ...
                ${rows.map(
                  (row) =>
                    `<tr key={row.id}>
                                <td>${row.supplier}</td>
                                <td>${row.speed}/10</td>
                                <td>${row.servicequality}/10</td>
                                <td>${row.reliability}/10</td>
                                <td>${row.deadline}/10</td>
                                <td>${row.totalpoint}</td>
                                <td>${row.average}</td>
                                <td>${row.calculation}</td>
                                <td>${row.evaluateperson}</td>
                                <td>${row.evaluate}</td>
                        </tr>`,
                )}
                </tbody>
  ...
  const handleInputChange = (index, field, value) => {
    const updatedRows = rows.map((row, i) => {
      if (i === index) {
        ...
        const totalFields = [
          parseFloat(updatedRow.speed || 0),
          parseFloat(updatedRow.servicequality || 0),
          parseFloat(updatedRow.reliability || 0),
          parseFloat(updatedRow.deadline || 0),
        ];

        const totalPoint = totalFields.reduce((sum, val) => sum + val, 0);
        updatedRow.totalpoint = totalPoint;

        const average =
          totalFields.length > 0 ? totalPoint / totalFields.length : 0;
        updatedRow.average = average.toFixed(2);

        const percentage = (totalPoint / 40) * 100;
        updatedRow.calculation = `${percentage.toFixed(2)}%`;
    ...
    (async () => {
      ...
      if (mode === DMode.Create) {
        await createDocument({
          title: title,
          shortName: "document",
          type: DocumentType.SupplierEvaluationForm,
          department: unit,
  ...
  return (
    <div className="container-fluid p-5">
      <div className="row justify-content-between">
        <h3 className="col-6 large-title">Tedarikçi Değerlendirme Formu</h3>
        <div
                  ...
                  <tr className="text-center" key={index}>
                    ...
                    <td>
                      <select
                        ...
                        value={row.speed}
                        onChange={(e) =>
                          handleInputChange(index, "speed", e.target.value)
                        }
                      >
                    ...
                    <td>
                      <input
                        ...
                        value={row.evaluate}
                        onChange={(e) =>
                          handleInputChange(index, "evaluate", e.target.value)
                        }
                      />
                    </td>
                  </tr>
