...
const ApplicationEvaluationForm = () => {
    ...
    const [tableData, setTableData] = useState(Array.from({ length: 10 }, () => Array(7).fill("")));
    ...
    if (!checkPermFromRole(user.roleValue, ActionPerm.DocumentViewAll)) return <UnauthPage />;
    ...
    const handleInputChange = (index, field, value) => {
        ...
        setState((prev) => ({ ...prev, row: updatedRows }));
    };

    const handleInputChangejoinperson = (index, value) => {
        const updatedJoinPerson = state.JoinPerson.map((item, i) =>
            i === index ? { ...item, value } : item
        );
        setState((prev) => ({ ...prev, JoinPerson: updatedJoinPerson }));
    };

    const addNewRow = () => {
        setState((prevState) => ({
            ...prevState,
            row: [...prevState.row, { id: Date.now(), XName: "", YName: "", person: "", sign: "" }],
        }));
    };
        ...
        if (mode !== DMode.Create) {
            ...
            (async () => {
                ...
                if (mode === DMode.ViewRevision) {
                    ...
                    doc = await getDocumentFromId(getId, true);
                ...
                if (doc.fields["points"]?.value) setState(JSON.parse(doc.fields["points"].value));
                if (doc.fields["table_data"]?.value) setTableData(JSON.parse(doc.fields["table_data"].value));

                // Alanlara verileri set et
                const elements = document.querySelectorAll("[field-short-name]");
                elements.forEach((element) => {
                    const fieldShortName = element.getAttribute("field-short-name");
                    if (doc.fields[fieldShortName]) element.value = doc.fields[fieldShortName].value;
                });
            })();
        }
    }, [mode, queryParameters]);
        ...
        (async () => {
            ...
            if (mode === DMode.Create) {
                await createDocument({
                    title,
                    shortName: "document",
                    type: DocumentType.ApplicationEvaluationForm,
                    department: unit,
    ...
    return (
        <div className="container-fluid p-5">
            <div className="row justify-content-between">
                <h3 className="col-6 large-title">Başvuru Değerlendirme Formu - 1</h3>
            ...
            <div className="row mt-5">
                <div className="col-12">
                    <div className="card" style={{ position: "sticky", top: "100px" }}>
                        <div className="card-header create-doc-card-header">Doküman Bilgileri</div>
                ...
                <div className="col-12 mt-5">
                    <div className="card" style={{ position: "sticky", top: "100px" }}>
                        <div className="card-header create-doc-card-header">Doküman İçeriği</div>
                        <div className="card-body justify-content-center px-5 row">
                            ...
                            >
                                <div className="row col-5">
                                    <label className="col-12 create-doc-p mb-3">Toplantı Başkanı</label>
                                    <input className="col-12 create-doc-inp" field-short-name="field1" type="text" />
                                </div>
                                <div className="row col-5">
                                    <label className="col-12 create-doc-p mb-3">Toplantı Yeri</label>
                                    <input className="col-12 create-doc-inp" field-short-name="field2" type="text" />
                                </div>
                                <div className="row col-5">
                                    <label className="col-12 create-doc-p mb-3">Toplantı Tarihi</label>
                                    <input className="col-12 create-doc-inp" field-short-name="field3" type="date" />
                                </div>
                                <div className="row col-5">
                                    <label className="col-12 create-doc-p mb-3">Toplantı Sekreteri</label>
                                ...
                                >
                                    <div className="row col-12 justify-content-between">
                                        ...
                                        >
                                            ...
                                            <button onClick={handleAddScore} className="col-2 print-btn2">
                                                + Ekle
                                            </button>
                                        </div>
                                    </div>
                                    ...
                                    >
                                        ...
                                        <tr style={{ backgroundColor: "#e6f7ff" }}>
                                            <td style={{ border: "1px solid #ccc", padding: "10px" }}>Puan Toplam</td>
                                                ...
                                                <tr key={index}>
                                                    ...
                                                    <td style={{ border: "1px solid #ccc", padding: "10px" }}>
                                                        <input
                                                            ...
                                                            value={item.value}
                                                            onChange={(e) => handleInputChangejoinperson(index, e.target.value)}
                                                        />
                                                    </td>
                                                </tr>
                                            ))}
