...
import SuiPopupComp from "./SuiPopup.jsx";
...
const Suitability = () => {
    ...
    useEffect(() => {
        const fetchDocuments = async () => {
            try {
                const result = await getAllDocuments(pageNum, 30, search, [DocumentType.Suitability],["field1","field2","field4","field5","field6","field7","date1"]);
    ...
    return (
        <div className="container-fluid pt-5">
            <div className="row justify-content-between" style={{padding: '0px 30px'}}>
                <h3 className="col-6 large-title">Uygunluk Değerlendirmeleri</h3>
                <div className="col-6 row justify-content-end " style={{columnGap: '10px'}}>
                    <button className="col-4 print-btn2 " onClick={togglePopup}>+ Değerlendirme Ekle</button>
                ...
                <div className="col-12">
                    ...
                    <thead>
                        <tr>
                            ...
                            <th className="text-center purple-text" scope="col">SAYI/TARİH</th>
                            <th className="text-center purple-text" scope="col">DÖKÜMAN ADI</th>
                            <th className="text-center purple-text" scope="col">TÜRÜ</th>
                            <th className="text-center purple-text" scope="col">MADDE NO</th>
                            <th className="text-center purple-text" scope="col">GEREKLİLİKLER / YÜKÜMLÜLÜKLER</th>
                            <th className="text-center purple-text" scope="col">MEVCUT DURUM</th>
                            <th className="text-center purple-text" scope="col">UYGUNLUK DURUMU</th>
                            <th className="text-center purple-text" scope="col">YAPILACAK FAALİYET / AKSİYON</th>
                            <th className="text-center purple-text" scope="col">SONUÇ VE DEĞERLENDİRME</th>
                        </tr>
                        </thead>
                        <tbody>
                        {documents.length > 0 ? (
                            documents.map((doc, index) => (
                                <tr key={index}>
                                    <td className="text-center">{doc.id || "Belirtilmemiş"}</td>
                                    <td className="text-center">{doc.fields?.date1?.value || "Belirtilmemiş"}</td>
                                    <td className="text-center">{doc.title || "Belirtilmemiş"}</td>
                                    <td className="text-center">{doc.fields?.field2?.value || "Belirtilmemiş"}</td>
                                    <td className="text-center">{doc.fields?.field1?.value || "Belirtilmemiş"}</td>
                                    <td className="text-center">{doc.fields?.field3?.value || "Belirtilmemiş"}</td>
                                    <td className="text-center">{doc.fields?.field4?.value || "Belirtilmemiş"}</td>
                                    <td className="text-center">{doc.fields?.field5?.value || "Belirtilmemiş"}</td>
                                    <td className="text-center">{doc.fields?.field6?.value || "Belirtilmemiş"}</td>
                                    <td className="text-center">{doc.fields?.field7?.value || "Belirtilmemiş"}</td>
                                </tr>
                            ))
                        ) : (
                            <tr>
